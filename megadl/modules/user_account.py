...
async def uptomega(client: Client, message: Message):
    ...
    megaupmsg = await megauplaod_msg.edit("**Trying to Download The Content to My Server! This may take while 😴**")
    mcli = MegaTools()
    ...
    if todownfile.media is None:
        try:
            ...
            if os.path.isdir(dl_path):
                ...
                loop = get_running_loop()
                toupload = await loop.run_in_executor(None, partial(wget.download, url, dl_path))
                link = await mcli.upload(toupload, megaupmsg.chat.id, megaupmsg.id)
